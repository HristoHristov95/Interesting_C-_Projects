...
public:
	...
	void display()
	{
		cout << "Display renewed account details " << no.data() << endl;
	}
};
...
public:
	LegacyAccountAdapter(string no) : LegacyAccount(atoi(no.c_str())) {}
...
int main()
{
	...
	list<RenewedAccountIntf*>::iterator p;
	while ((p=accountList.begin()) != accountList.end())
	{
		(*p)->display();
